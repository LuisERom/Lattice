...
export function summarizePhaseOutcome(phase: string, rawPayload: unknown): OutcomeSection[] {
  ...
  if (phase === "B_seeds") {
    return [{ title: "Learning goals", lines: [`Seeds: ${numberCount(payload.seeds)}`] }];
  }
  ...
  if (phase === "C_expand") {
    const stats = payload.stats && typeof payload.stats === "object" ? (payload.stats as JsonObject) : {};
    return [
      {
        title: "Prerequisite expansion",
        lines: [
          `Nodes: ${numberCount(payload.nodes)}`,
          `Prerequisite edges: ${typeof stats.prereq_edges === "number" ? stats.prereq_edges : numberCount(payload.edges)}`,
        ],
      },
    ];
  }
